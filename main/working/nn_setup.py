import sys
sys.path.append('../technicalTrading')

from packages.analyze.src.fetch import Fetch
# from strategy import Indicators
from packages.utility.src.utility import Utility
...
def trainingData(data,minHold=None,minDelta=None):
    ...
    prices = [data.Close[np.asarray(extrema).transpose()[0].astype(int)].to_numpy() for extrema in [troughs,peaks]]
...
_, ax = plt.subplots()
ax.plot(xvals,data.Close,linewidth=0.5)
ax.plot(xvals,data.Smooth,color='black',linewidth=1)
ax.scatter(dates[0],prices[0],c='g')
ax.scatter(dates[1],prices[1],c='r')

ax2 = ax.twinx()
ax2.scatter(xvals,buy,c=buy,cmap='RdYlGn')
